...
static class GlobalConsts
{
    ...
    static GlobalConsts()
    {
        ...
        var appDataPath = string.Concat(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "\\LisTube\\");
        ...
        TempFolderPath = string.Concat(Path.GetTempPath(), "LisTube\\");
        UpdateOnExit = false;
        UpdateLater = false;
    ...
    public static void CreateTempFolder()
    {
        try
        {
            if (!Directory.Exists(Path.GetTempPath() + "LisTube"))
                Directory.CreateDirectory(Path.GetTempPath() + "LisTube");
        }
        catch (Exception ex)
        {
    ...
    public static void CleanTempFolder()
    {
        if (Directory.Exists(Path.GetTempPath() + "LisTube"))
        {
            DirectoryInfo di = new(Path.GetTempPath() + "LisTube");
